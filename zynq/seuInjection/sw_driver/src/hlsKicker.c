...
int main(int argc, char *argv[])
{
    ...
    *((int*)(hlsdev)) = 0;
    ...
    //check for errors
    if(*((int*)(hlsdev)+4) != 0){
        printf("%s, \t%d, \t%d, \t%d, \t%d, \t%d, \t%d, \t%d, \tERROR\n", linearaddr, *((int*)(hlsdev)),*((int*)(hlsdev)+1),*((int*)(hlsdev)+2),*((int*)(hlsdev)+3),*((int*)(hlsdev)+4), *((int*)(hlsdev)+5), *((int*)(hlsdev)+6));
        //printf("CFG Error detected\n");
        return 1;
    }

    //printf("Circuit completed successfully ans=%d\n", *((int *)(hlsdev)+3));
    printf("%s, \t%d, \t%d, \t%d, \t%d, \t%d, \t%d, \t%d, \tOKAY\n", linearaddr, *((int*)(hlsdev)),*((int*)(hlsdev)+1),*((int*)(hlsdev)+2),*((int*)(hlsdev)+3),*((int*)(hlsdev)+4), *((int*)(hlsdev)+5), *((int*)(hlsdev)+6));
	return 0;
}
